...
const repo = { async getState(id) { return { customEx: [], owner: id, theme: 'light' }; }, async saveState(id, state) { return { ...state, owner: id }; } };
...
test('athlete service scopes reads and writes to the actor', async () => {
  ...
  assert.equal((await service.getState(actor)).theme, 'light');
