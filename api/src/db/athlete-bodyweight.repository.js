...
export class AthleteBodyweightRepository {
  ...
  async list(athleteId, { limit = 100 } = {}) {
    ...
    return result.rows.reverse().map(row => ({ d: new Date(row.d).toISOString().slice(0, 10), w: Number(row.w) }));
  }
  async save(athleteId, point) {
    ...
    return { d: new Date(result.rows[0].d).toISOString().slice(0, 10), w: Number(result.rows[0].w) };
  }
}
