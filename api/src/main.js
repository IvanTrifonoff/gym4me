...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (req.method === 'GET' && pathname(req) === '/api/v1/athlete/me') return sendJson(res, 200, { data: await service.getMe(actor) });
  ...
  } catch (error) { return sendJson(res, error.status || 500, { error: error.message || 'internal error' }); }
});
