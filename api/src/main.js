...
import { migrateAthleteBodyweight } from './db/athlete-bodyweight.migration.js';
...
if (process.env.GYM4ME_START_SERVER === '1') { const start = async () => { if (bodyweightRepository) await migrateAthleteBodyweight(pool); server.listen(config.port, () => console.log(`opengym-next api on :${config.port}`)); }; start().catch(error => { console.error(error); process.exitCode = 1; }); }
